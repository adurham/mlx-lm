...
class Model(nn.Module):
    ...
    def sanitize(self, weights: Dict[str, mx.array]) -> Dict[str, mx.array]:
        ...
        top_remap = {
            ...
            # Quantized embed: mlx-community quants ship `embed.{scales,biases}`
            # alongside `embed.weight`. Without renaming these too, nn.quantize's
            # `f"{p}.scales" in weights` predicate misses and the embedding
            # stays as plain nn.Embedding while the .weight is the int4-packed
            # tensor — shape/dtype mismatch at first forward.
            "embed.scales": "model.embed_tokens.scales",
            "embed.biases": "model.embed_tokens.biases",
            "norm.weight": "model.norm.weight",
            "head.weight": "lm_head.weight",
            # Quantized lm_head — same reasoning as embed above.
        ...
        for k, v in weights.items():
            ...
            for old, new in w_remap.items():
                # `shared_experts` first so its substring isn't partially
                # rewritten by the routed-experts replace below.
                nk = nk.replace(f".shared_experts.{old}.", f".shared_experts.{new}.")
                # mlx-community quants of DSv4 ship the 256-expert tensor
                # already stacked under `experts.{w1,w2,w3}.` rather than
                # the per-expert unstacked layout the loop below expects.
                # Map straight to the model's `switch_mlp.{gate,down,up}_proj`
                # so load_weights doesn't silently drop them under
                # strict=False (which would leave the SwitchLinear at its
                # ~8.6 GB-per-projection random fp32 init and OOM the box).
                nk = nk.replace(
                    f".ffn.experts.{old}.", f".ffn.switch_mlp.{new}."
                )
            remapped[nk] = v
        weights = remapped
        ...
        # mlx-community DSv4 quants ship `wo_a` as o_groups separate
        # sub-tensors `wo_a.{0..o_groups-1}.{weight,scales,biases}` — one per
        # output group. V4Attention's `self.wo_a` is a single nn.Linear of
        # shape (o_groups * o_lora_rank, in), so the model's parameter tree
        # has `wo_a` as a leaf with `weight/scales/biases` and load_weights
        # crashes with KeyError when tree_unflatten produces a list of dicts
        # (per-group leaves) where the model expects a flat one. Concat the
        # per-group tensors along the output dim so the merged tensor matches
        # the model's expected shape. Safe to run unconditionally — only
        # fires if the per-group keys are present.
