    ...
    *,
    ...
    def _step(input_tokens: mx.array, input_embeddings: Optional[mx.array] = None):
        ...
        with mx.stream(stream):
            ...
            logprobs = logits.astype(mx.float32) - mx.logsumexp(logits.astype(mx.float32), keepdims=True)
    ...
    with mx.stream(stream):
        ...
        while total_prompt_tokens - prompt_processed_tokens > 1:
            ...
            # Upstream: keep the logits-processor history in sync with the
            # tokens consumed during prefill (ml-explore/mlx-lm#1777).
        ...
        # Upstream moved the decode loop inside the `with mx.stream(stream)`
        # block (ml-explore/mlx-lm#1775, thread safety); the fork's
        # MLX_GPU_TIME probe instrumentation is re-indented to match.
        n = 0
        while True:
            if _gpu_probe:
