...
def compute_g(A_log, a, dt_bias):
    # Fork delta (adurham/mlx-lm, was 5a4de96 + e6e3101): promote the softplus
    # argument to float32 before the gate is computed. Upstream only promotes
    # the outer multiply, so `a + dt_bias` and its softplus run at the model
    # dtype (bf16), which compounds into GDN state drift over long decodes and
    # diverges from the vLLM/Triton reference. Keeping it in f32 here matches
    # the reference and costs a couple of elementwise casts.
    return mx.exp(
...
def compute_lower_bound_g(A_log, a, dt_bias, lower_bound):
    # dt_bias promoted to f32 for the same reason as in compute_g; upstream
    # already promotes `a` here.
    return mx.exp(
        lower_bound
        * mx.sigmoid(
            mx.exp(A_log.astype(mx.float32))
...
def gated_delta_update(
    ...
    """
    # Fork delta: sigmoid in float32. In bf16 the beta gate quantises coarsely
    # and the error accumulates in the recurrent state; f32 matches the
    # vLLM/Triton reference. `g` and `beta` are untemplated kernel buffers
    # (the Metal kernels template only InT=q/k/v and StT=state), so a float32
    # beta is bound as a float buffer and read correctly — upstream already
    # passes a float32 `g` alongside bf16 q/k/v for the packed path.
